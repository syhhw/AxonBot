...
async def cmd_addtrigger(client, message):
    ...
    if len(matches) < 2:
        return await message.edit_text(tr(f"⚠️ Use: `{p}addtrigger \"palavra\" \"resposta\"`", f"⚠️ Use: `{p}addtrigger \"word\" \"response\"`"))
    ...
    await message.edit_text(tr(f"✅ **Trigger salvo!**\nSe disserem: `{gatilho}`\nResponderei: `{resposta}`", f"✅ **Trigger saved!**\nIf they say: `{gatilho}`\nI'll reply: `{resposta}`"))
